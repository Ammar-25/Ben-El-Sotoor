...
namespace BaynAlSutoor.Application.Services
{
    public class ProfileService
    {
        ...
        public async Task<UserProfileDto?> GetMyProfileAsync(int userId)
        {
            ...
            //     .FirstOrDefault(s => s.UserId == user.Id && s.IsActive && s.EndDate >= System.DateTime.UtcNow);
        ...
        public async Task<IEnumerable<ReviewDto>> GetMyReviewsAsync(int userId)
        {
            var reviews = await _unitOfWork.Reviews.FindAsync(r => r.UserId == userId);
            var reviewList = reviews.ToList();
            foreach (var r in reviewList)
            {
                r.User = await _unitOfWork.Users.GetByIdAsync(userId) ?? new User { Name = "Anonymous" };
                r.Book = await _unitOfWork.Books.GetByIdAsync(r.BookId);
            }
            return _mapper.Map<IEnumerable<ReviewDto>>(reviewList.OrderByDescending(r => r.Date));
        }
    }
}
