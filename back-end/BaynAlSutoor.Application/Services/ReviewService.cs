...
namespace BaynAlSutoor.Application.Services
{
    public class ReviewService
    {
        ...
        public async Task<PaginatedReviewDto> GetReviewsByBookIdAsync(int bookId, int page = 1, int limit = 5)
        {
            ...
            {
                Items = _mapper.Map<List<ReviewDto>>(pagedReviews),
        ...
        public async Task<ReviewSummaryDto> GetReviewSummaryAsync(int? bookId)
        {
            ...
            {
                r.User = await _unitOfWork.Users.GetByIdAsync(r.UserId) ?? new User { Name = "Anonymous" };
                r.Book = await _unitOfWork.Books.GetByIdAsync(r.BookId);
            }
        ...
        public async Task<IEnumerable<ReviewDto>> GetTopReviewsAsync(int limit)
        {
            var reviews = await _unitOfWork.Reviews.FindAsync(r => r.Rating == 5);
            var topReviews = reviews.OrderByDescending(r => r.Date).Take(limit).ToList();
            
            foreach (var r in topReviews)
            {
                r.User = await _unitOfWork.Users.GetByIdAsync(r.UserId) ?? new User { Name = "Anonymous" };
            }
            return _mapper.Map<IEnumerable<ReviewDto>>(topReviews);
        }
    }
}
